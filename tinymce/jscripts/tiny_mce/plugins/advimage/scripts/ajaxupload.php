<?php
	function uploadImage($fileName, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH = null){
		...
		if(sizeof($errorList) == 0){
			...
			$url = str_replace("/image.htm?8","/uploads/",$_SERVER['HTTP_REFERER']);
...
<?php
	...
	//echo $upload_image;
	echo '<input type="hidden" value="'.$upload_image.'" name="src" id="src"/>';
	echo '<br /><label id="altlabel" for="alt">Image Description</label> 

		<input id="alt" name="alt" type="text" value="" /><br />
		<label id="titlelabel" for="title">Image title</label>

		<br /><input id="title" name="title" type="text" value="" /><br />';
	?>
	
	<?
	echo '<div id="prev" style="width:300px; margin-top:10px;"><img id="previewImg" src="'.$upload_image.'" border="0" onload="ImageDialog.updateImageData(this, 1);" /></div>';
 
		 echo '<div id="imageName" style=" display:none;">'.$upload_image.'</div><img src="images/success.gif" width="16" height="16" border="0" style="marin-bottom: -4px;" /> Success!<br />';
	}else{
