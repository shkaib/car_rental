    ...
    tjq('.datepicker-wrap input').each(function() {
        ...
        tjq(this).datepicker({
            ...
            onClose: function(selectedDate, test) {
				...
				if(tjq(this).attr('name') == 'pickupdate'){
				  ...
				  var MyDateString = ('0' + (tempDate.getMonth()+1)).slice(-2) + '/' + ('0' + (parseInt(tempDate.getDate()))).slice(-2) + '/' + tempDate.getFullYear();
				 //alert(MyDateString);
				  tjq("#returndate").datepicker( "setDate", MyDateString);
				}
            }
        });
    });
...
tjq(window).load(function() {
    ...
    if (enableChaser == 1 && tjq('#content').length > 0 && tjq('#main-menu ul.menu').length > 0) {
        ...
        tjq('<h1 class="logo navbar-brand"><a title="Trans Auto Rental - home" href="#"><img alt="" src="images/logo_admin.png"></a></h1>').insertBefore('.chaser .menu');
...
$(document).on("change","#pickuptime",function(){
	
	var picktime = $(this).val();
	$('.valreturn span.custom-select').text(picktime);
	$('#returntime').val(picktime);
	});
	
$('.btn-show-form').click(function(e){
	e.preventDefault();
	$('.more-search-form').toggle('slow');
	var txt = $(this).find('em');
	txt.html() == "Open" ? txt.html('Close') : txt.html('Open');
	$(this).toggleClass('chev-up chev-down');
	})	
$('#txt_pickup_location').click(function(e){
	e.preventDefault();
	$('.more-search-form').show('slow');
	var txt = $('.btn-show-form').find('em');
	if($('.more-search-form').is(':visible'))
	{
	  txt.html('Close');
	  $('.btn-show-form').addClass('chev-up').removeClass('chev-down');
	}
	else {
		txt.html('Open');
		$('.btn-show-form').addClass('chev-down').removeClass('chev-up');
		}
	})		