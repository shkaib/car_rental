			...
			<div class="content-block" role="main">
              ...
              <thead>
                <tr>
										...
										<tbody>
								<?php
								$objProduct->setProperty("type_id", 1);
                                ...
                                ?>
                                <tr class="odd gradeX">
                                    <td><a href="./?p=lease_form&mode=view&lease_id=<?php echo EncData($rows['lease_id']);?>" title="<?php echo $rows['full_name'];?>"><?php echo $rows['full_name'];?></a></td>
                                    <td><?php echo LeaseType($rows['lease_type']);?></td>
                                    <td><?php if($rows['lease_ststus']==1){ echo 'New'; } else { echo 'Read';}?></td>
                                    <td><a href="./?p=lease_mgmt&mode=delete&lease_id=<?php echo EncData($rows['lease_id']);?>" onClick="return doConfirm('Are you sure you want to delete the <?php echo $rows['location_title'];?>?');" title="Delete">
