...
class Common extends Database{
	...
	*/
	public function displayMessage($flag = true){
		if($data = $this->getMassage()){
			...
			$output = 	'<div style="background-color: #ACD6AC;
	...
	* @modified 14 Dec, 2007 by Numan Tahir
	*/
	public function displayMessage_2(){
		if($data = $this->getMassage()){
			$output = '';
			foreach($data as $type => $msg){
				if(count($msg) >= 1 && $type == 'Error'){
	...
	* @modified 14 Dec, 2007 by Numan Tahir
	*/
	public function DisplayMessageNew($flag = true){
		if($data = $this->getMassage()){
			$output = '';
			...
			foreach($data as $type => $msg){
				if(count($msg) >= 1 && $type == 'Error'){
					foreach($msg as $msg){
						$output .= 	'<div class="NotificationErrorMsg" id="notification_div">';
						$output .=	$msg ;
					}
				}
				else if(count($msg) > 1 && $type == 'Info'){
					foreach($msg as $msg){
						$output .= 	'<div class="NotificationSuccessMsg" id="notification_div">';
						$output .= $msg;
					}
				}
		  		else{
