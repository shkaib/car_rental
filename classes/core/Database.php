...
class Database extends Property{
	...
	*/
	public function dbQuery($sql = ""){
		...
		if(!$this->rsResource){
			...
			die(mysql_error());
			//die('<div style="width:100%; float:left; text-align:center; margin-top:10px; font-size:12px;">Sorry no result found.</div>');
			return false;
		}
		else{
			$this->sql = $sql;
	...
	*/
	...
	/**
	* This method is used to append the limiting Start and End sql
	* @author Numan Tahir
	* @Date : 07 Nov, 2013
	* @modified :  07 Nov, 2013 by Numan Tahir
	* @return : bool
	*/
	public function appendLimitSet($startend){
		list($start,$perpage)= explode(',', $startend);
		$page = isset($perpage) ? trim($perpage) : 1;
		$start = isset($start) ? trim($start) : 0;
		$Sql = " LIMIT $start, $perpage";
		return $Sql;
	}
